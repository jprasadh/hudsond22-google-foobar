...
def count_patterns(s):
    ...
    while i <= len(s):
        ...
        if len(pattern) * occurrences == len(s):
            pattern_valid = True
        else:
            pattern_valid = False
        if occurrences > result and pattern_valid:
            result = occurrences
        i += 1
    return result
...
def answer(s):
    ...
    i = 0
    while i < len(s):
        if count_patterns(s) > max_occurrences:
            max_occurrences = count_patterns(s)
        s = rotate(s, 1)
        i += 1
